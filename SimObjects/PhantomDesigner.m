classdef PhantomDesigner < handle
    ...
    methods
        function this = PhantomDesigner()
            this.vars.muaWater = 0.00258; % [mm^-1] from [2]
            this.vars.muaMin   = this.vars.muaWater;
            this.vars.moldSize = 60; % mm;
            fprintf("The absorption coeff. of water in 785nm is MuA = %.4f [1/mm] [2]\n", this.vars.muaWater);
        end
        ...
        function calcWaterRefractibeIndex(this)
            % According to Eq. (3) on [3] (IAPWS method)
            I = 1.311;
            J = 1.154e4;
            K = -1.132e9;
            
            n = I + J/lambda^2 + K/lambda^4;
        end
        ...
        function calcInkProperties(this, wl)
            ...
            switch this.vars.inkType
                case 'RoyalTalens'
                    ...
                    fprintf("The absorption coeff. of 1:100 ink-base (Royal Talens) in %d nm is MuA = %.3f [1/mm] [1]\n\n", wl, this.vars.muaInkBase);
            end
        end
        ...
        function res = createPhantomSet(this, uVars)
            fprintf("Generic Phantom Set Recipe: \n\n");
            ...
            for j = 1:numLayers
                ...
                for i = 1:numPhantom
                    scatBase    = this.calcLipid20Vol(this.vars.musP(j,i), vCurLayer);
                    vWaterLayer(j,i)    = vCurLayer;
                    vLipid20Layer(j,i)  = scatBase.vLipid20;
                    vScatBaseLayer(j,i) = scatBase.vTotal;

                    resAbs  = this.calcInkVolLayer(muaPh(j,i), scatBase.vTotal);
            ...
            vWaterTotal = sum(vWaterLayer(:)) + sum(vWaterLayer(:,1));
            vLipidTotal = sum(vLipid20Layer(:));
            this.calcLipid20Base(vLipidTotal);
            
            fprintf("To create %2.f [ml] of Agarose Mixture, mix: \n", vWaterTotal);
            ...
            phIdx   = 1:numPhantom;
            muaGT   = muaPh;
            musPGT  = this.vars.musP;
            muEffGT = this.vars.muEff;
            MFPGT   = this.vars.MFP;
            
            for j=1:this.vars.numLayers
                fprintf("Layer %d:\n", j)
                Input = table(phIdx', muaGT(j,:)', musPGT(j,:)', muEffGT(j,:)', MFPGT(j,:)','VariableNames', varNamesInput);
                disp(Input)
            ...
            res.GT.mua   = muaGT;
            res.GT.musP  = musPGT;
            res.GT.muEff = muEffGT;
            res.GT.MFP   = MFPGT;
            
            res.op.mua   = muaPh;
            res.op.mus   = musPh;
            res.op.musP  = musPPh;
            res.op.muEff = muEffPh;
            res.op.MFP   = MFPPh;
            
            res.vol.water   = vWaterLayer;
            res.vol.inkBase = vInkBase;
            res.vol.lipid20 = vLipid20Layer;
            res.vol.total   = totalVol;
        end
        
        function gradedAbsorption(this, uVars)
            fprintf("Graded Absorption Set Recipe: \n\n");
        ...
        function gradedScattering(this, uVars)
            fprintf("Graded Scattering Set Recipe: \n\n");
            ...
            varNamesInput = {'Phantom idx' ; 'MuA'; 'MuSP'; 'MuEff'; 'MFP'};
            varNamesOP    = {'Phantom idx' ; 'MuA'; 'MuS'; 'MuSP'; 'MuEff'; 'MFP'};
            varNamesVols  = {'Phantom idx' ; 'V-Water'; 'V-Lipid 20%%'; 'V-AbsBase'; 'V-InkBase'; 'Total Volume'};
